...
public class LoadingManager : MonoBehaviour
{
    ...
    void AnimationGoLeft(GameObject gameObject, int i, float moreSize = 0)
    {
        ...
        gameObject.transform.DOMove(new Vector2(_center.transform.position.x / 3f, _center.transform.position.y - (i * 120) - moreSize), duration).SetEase(Ease.OutCubic);
    ...
    private void Update()
    {
        ...
        for (int i = 0; i < _listButtons.Count; i++)
        {
            ...
            Vector2 buttonPosition = new Vector2(_listButtons[i].transform.position.x, _listButtons[i].transform.position.y);
            ...
            //Debug.Log("MOUSE POSITION " +mousePosition2D.ToString());
            //Debug.Log("BUTTON POSITION " + buttonPosition.ToString());

            if (buttonPosition.x - size  < mousePosition2D.x &&
                buttonPosition.x + size > mousePosition2D.x &&
                buttonPosition.y - size < mousePosition2D.y &&
                buttonPosition.y + size > mousePosition2D.y)
            {
                _selectedButton = i;
                ChangeColors();
